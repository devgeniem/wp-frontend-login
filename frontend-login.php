...
class Frontend_Login {
	...
	public function check_login() {
		if ( ! is_user_logged_in() ) {
			if ( preg_match( "/frontend_login_token=([^&]+)&?/", $_SERVER["REQUEST_URI"], $matches ) ) {
				...
				$users = get_users( array( "meta_key" => "_frontend_login_token",
				...
				delete_user_meta( $user->ID, "_frontend_login_token" );
				delete_user_meta( $user->ID, "_frontend_login_timestamp" );

				return;
			}
		}
		else if ( preg_match( "/frontend_logout/", $_SERVER["REQUEST_URI"] ) ) {
			wp_logout();
		}
	}

	public function admin_stuff() {
		if ( preg_match( "/frontend_logout/", $_SERVER["REQUEST_URI"] ) ) {
			wp_logout();
		}
		else if ( isset( $_SESSION["frontend_login_done"] ) ) {
			return;
		}
		else {
			...
			wp_localize_script( "frontend_login", "frontend_login_data", $data );

			// enqueue the javascript
			wp_enqueue_script("frontend_login");

			// insert the token into database
			add_user_meta( $user_id, "_frontend_login_token", $token, true );

			// insert timestamp into database
			add_user_meta( $user_id, "_frontend_login_timestamp", time(), true );

			$_SESSION["frontend_login_done"] = true;
		}
	}

	public function logout() {
		unset($_SESSION["frontend_login_done"]);
	}
	...
	public function custom_login() {
		if ( preg_match( "/loggedout/", $_SERVER["REQUEST_URI"] ) ) {
			...
			if ( preg_match( "/admin/", $_SERVER['SERVER_NAME'] ) ) {
				$data["url"] = WP_PUB_SITEURL ."/?frontend_logout";	
			}
			else {
				$data["url"] = WP_ADMIN_URL ."/?frontend_logout";
			}
